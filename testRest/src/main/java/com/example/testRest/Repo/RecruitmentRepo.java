...
public interface RecruitmentRepo extends BaseRepo<Recruitment>{
    @Query(value = "proc_get_recruitment_home", nativeQuery = true)
    public List<Recruitment> getHomes();
    
    @Query(value = "proc_get_recruitment_hot", nativeQuery = true)
    public List<Recruitment> getHots();
    
    @Query(value = "proc_get_recruitments_filter ?1,?2,?3", nativeQuery = true)
