...
public interface ProjectRepo extends BaseRepo<Project>{
    ...
    @Query(value = "proc_get_project_home", nativeQuery = true)
    public List<Project> getHomes();
    
    @Query(value = "proc_get_project_hot", nativeQuery = true)
    public List<Project> getHots();
    
    @Query(value = "proc_get_projects_filter ?1,?2,?3", nativeQuery = true)
